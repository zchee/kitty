...
prefetch_line_for_write(HistoryBuf *self, index_type y) {
    ...
    if (y >= self->positions_allocated) return; // must not trigger position allocation
...
historybuf_take_line_from(HistoryBuf *self, LineBuf *lb, index_type lb_y, ANSIBuf *as_ansi_buf) {
    ...
    // S1/S2 (Phase 13B): if the evicted row still carries a deferred clear,
    // finalize it now so the scrollback slot holds authoritative GPUCells —
    ...
    // never eligible — Architect R2 M-NEW-1(c)).
    linebuf_gen_bump(lb, lb_p);
}
