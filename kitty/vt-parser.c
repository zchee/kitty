...
run_worker(void *p, ParseData *pd, bool flush) {
    ...
    // Wave-25 Lane S: process-cumulative counters (the retire fires inside
    // line-buf/history helpers with no Screen at hand)
    const uint64_t share_total0 = share_rows_total_counter, share_ref0 = share_rows_ref_counter, share_ret0 = share_cow_retires_counter;
    ...
    pd->share_rows_total = (unsigned)(share_rows_total_counter - share_total0);  // Wave-25 Lane S
    pd->share_rows_ref = (unsigned)(share_rows_ref_counter - share_ref0);
    pd->share_cow_retires = (unsigned)(share_cow_retires_counter - share_ret0);
