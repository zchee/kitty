...
#define n(which, attr)                                                                    \
    ...
    n(default_bg, background);
    n(cursor_color, cursor);
    n(cursor_text_color, cursor_text_color);
    n(highlight_fg, selection_foreground);
    n(highlight_bg, selection_background);
    n(visual_bell_color, visual_bell_color);
#undef n
    RAII_PyObject(src, PyObject_GetAttrString(opts, "transparent_background_colors"));
    if (!src) {
        PyErr_SetString(PyExc_TypeError, "No transparent_background_colors on opts object");
        return false;
    }
...
static bool
is_generated_color(const color_type c) {
    return (c & GENERATED_COLOR_MASK) == GENERATED_COLOR_MASK;
}
...
palette_generation_is_dynamic(PyObject *opts, bool *semantic) {
    ...
    *semantic = false;
    if (opts) {
        RAII_PyObject(policy, PyObject_GetAttrString(opts, "palette_generate"));
        if (!policy) return ans;
...
set_colortable(ColorProfile *self, PyObject *opts) {
    ...
    if (!color_table || count != arraysz(FG_BG_256)) {
        PyErr_SetString(PyExc_TypeError, "color_table has incorrect length");
        return false;
    }
    RAII_PyObject(r2, PyObject_GetAttrString(ct, "itemsize"));
    if (!r2) return false;
    size_t itemsize = PyLong_AsSize_t(r2);
    if (itemsize != sizeof(unsigned long)) {
        PyErr_Format(PyExc_TypeError, "color_table has incorrect itemsize: %zu", itemsize);
        return false;
    }
...
patch_color_profiles(PyObject *module UNUSED, PyObject *args) {
    PyObject *spec, *transparent_background_colors, *profiles, *v;
    ColorProfile *self;
    int change_configured;
    if (!PyArg_ParseTuple(args, "O!O!O!p", &PyDict_Type, &spec, &PyTuple_Type, &transparent_background_colors, &PyTuple_Type, &profiles, &change_configured))
        return NULL;
    char key[32] = {0};
...
#define S(config_name, profile_name)                                      \
    ...
    S(selection_foreground, highlight_fg);
    S(selection_background, highlight_bg);
    S(cursor_text_color, cursor_text_color);
    S(visual_bell_color, visual_bell_color);
#undef SI
#undef S
    for (Py_ssize_t i = 0; i < PyTuple_GET_SIZE(profiles); i++) {
        self = (ColorProfile *)PyTuple_GET_ITEM(profiles, i);
    ...
    *opacity = 1.0;
    if (index < arraysz(self->configured_transparent_colors)) {
        if (self->overriden_transparent_colors[index].is_set) {
            *color = self->overriden_transparent_colors[index].color;
            *opacity = self->overriden_transparent_colors[index].opacity;
            if (*opacity < 0) *opacity = OPT(background_opacity);
            return true;
        }
        if (self->configured_transparent_colors[index].is_set) {
            *color = self->configured_transparent_colors[index].color;
            *opacity = self->configured_transparent_colors[index].opacity;
            if (*opacity < 0) *opacity = OPT(background_opacity);
            return true;
        }
    }
...
colorprofile_to_color(const ColorProfile *self, DynamicColor entry, DynamicColor defval) {
    switch (entry.type) {
        case COLOR_NOT_SET: return defval;
        case COLOR_IS_INDEX: {
            DynamicColor ans;
        ...
        case COLOR_IS_RGB:
        case COLOR_IS_SPECIAL: return entry;
    }
    return entry;
}
...
colorprofile_to_color_with_fallback(ColorProfile *self, DynamicColor entry, DynamicColor defval, DynamicColor fallback, DynamicColor fallback_defval) {
    switch (entry.type) {
        ...
        case COLOR_IS_SPECIAL:
            ...
            return defval.rgb;
        case COLOR_IS_RGB: return entry.rgb;
        case COLOR_IS_INDEX: return self->color_table[entry.rgb & 0xff] & (~GENERATED_COLOR_MASK);
    }
    return entry.rgb;
}
...
#define D(attr, name)                                                                                            \
    ...
    D(default_fg, foreground);
    D(default_bg, background);
    D(cursor_color, cursor);
    D(cursor_text_color, cursor_text);
    D(highlight_fg, selection_foreground);
    D(highlight_bg, selection_background);
    D(visual_bell_color, visual_bell_color);
...
#define as_color_doc "Convert the specified terminal color into an (r, g, b) tuple based on the current profile values"
    if (!PyLong_Check(val)) {
        PyErr_SetString(PyExc_TypeError, "val must be an int");
        return NULL;
    }
...
copy_color_table_to_buffer(ColorProfile *self, color_type *buf, int offset, size_t stride) {
    ...
    for (i = 0; i < arraysz(self->mark_backgrounds); i++) {
        *buf = self->mark_backgrounds[i];
        buf += stride;
    }
    for (i = 0; i < arraysz(self->mark_foregrounds); i++) {
        *buf = self->mark_foregrounds[i];
        buf += stride;
    }
    self->dirty = false;
}
...
colorprofile_reset(ColorProfile *self) {
    ...
    for (unsigned i = 0; i < arraysz(self->overriden_transparent_colors); i++) { zero_at_ptr(self->overriden_transparent_colors + i); }
    for (unsigned i = 0; i < self->color_stack_sz; i++) { zero_at_ptr(self->color_stack + i); }
}
...
static PyGetSetDef cp_getsetters[] = {
    GETSET(default_fg) GETSET(default_bg) GETSET(cursor_color) GETSET(cursor_text_color) GETSET(highlight_fg) GETSET(highlight_bg)
        GETSET(visual_bell_color){NULL} /* Sentinel */
};


static PyMemberDef cp_members[] = {{NULL}};
...
palette_color_is_generated(ColorProfile *self, PyObject *x) {
    ...
    if (idx >= arraysz(FG_BG_256)) {
        PyErr_SetString(PyExc_IndexError, "idx out of bounds");
        return NULL;
    }
...
get_transparent_background_color(ColorProfile *self, PyObject *index) {
    if (!PyLong_Check(index)) {
        PyErr_SetString(PyExc_TypeError, "index must be an int");
        return NULL;
    }
    unsigned long idx = PyLong_AsUnsignedLong(index);
    if (PyErr_Occurred()) return NULL;
    if (idx >= arraysz(self->configured_transparent_colors)) Py_RETURN_NONE;
    TransparentDynamicColor *c =
        self->overriden_transparent_colors[idx].is_set ? self->overriden_transparent_colors + idx : self->configured_transparent_colors + idx;
...
set_transparent_background_color(ColorProfile *self, PyObject *const *args, Py_ssize_t nargs) {
    if (nargs < 1) {
        PyErr_SetString(PyExc_TypeError, "must specify index");
        return NULL;
    }
    ...
    if (idx >= arraysz(self->configured_transparent_colors)) Py_RETURN_NONE;
    if (nargs < 2) {
        self->overriden_transparent_colors[idx].is_set = false;
        Py_RETURN_NONE;
    }
    if (!PyObject_TypeCheck(args[1], &Color_Type)) {
        PyErr_SetString(PyExc_TypeError, "color must be Color object");
        return NULL;
    }
...
static PyMethodDef cp_methods[] = {
    METHOD(reset_color_table, METH_NOARGS) METHOD(as_dict, METH_NOARGS) METHOD(basic_colors, METH_NOARGS) METHOD(color_table_address, METH_NOARGS)
        METHOD(as_color, METH_O) METHOD(reset_color, METH_O) METHOD(set_color, METH_VARARGS) METHODB(palette_color_is_generated, METH_O),
...
PyTypeObject ColorProfile_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "fast_data_types.ColorProfile",
    ...
    case ch:                                                                                        \
        ...
        break;
            switch (name[0]) {
                ...
                C('b', 2, "blue");
                C('a', 3, "alpha");
                default: PyErr_Format(PyExc_TypeError, "Color() got an unexpected keyword argument '%s'", name); return NULL;
            }
#undef C
            if (idx < nargs) {
...
contrast(Color *self, PyObject *o) {
    if (!PyObject_TypeCheck(o, &Color_Type)) {
        PyErr_SetString(PyExc_TypeError, "Not a Color");
        return NULL;
    }
    Color *other = (Color *)o;
...
hexchar_to_int(char c) {
    switch (c) {
        ...
        case 'A' ... 'F': return c - 'A' + 10; END_ALLOW_CASE_RANGE
    }
    return -1;
}
...
parse_sharp(const char *spec, size_t len) {
    ...
    switch (len) {
        ...
        case 9:
        case 12:
            if (!parse_single_color(spec, 2, &r) || !parse_single_color(spec + len / 3, 2, &g) || !parse_single_color(spec + 2 * len / 3, 2, &b))
                Py_RETURN_NONE;
            break;
        default: Py_RETURN_NONE;
    }
...
as8bit(double f) {
    return (unsigned char)(round(f * 255.));
}
...
oklch_to_srgb_gamut_map(double l, double c, double h, double *r, double *g, double *b) {
    ...
    if (!isfinite(l) || !isfinite(c) || !isfinite(h) || l <= 0) {
        *r = 0;
        *g = 0;
        *b = 0;
        return;
...
#define in_gamut(r, g, b) (0. <= r && r <= 1. && 0. <= g && g <= 1. && 0. <= b && b <= 1.)
    ...
    while ((high_chroma - low_chroma) > MIN_CONVERGENCE) {
        ...
        if (in_gamut(r_test, g_test, b_test)) {
            ...
            r_clipped = clamp(r_test);
            g_clipped = clamp(g_test);
            b_clipped = clamp(b_test);
...
parse_color(PyTypeObject *type UNUSED, PyObject *pspec) {
    if (!PyUnicode_Check(pspec)) {
        PyErr_SetString(PyExc_TypeError, "spec must be a string");
        return NULL;
    }
...
static PyMethodDef color_methods[] = {
    METHODB(contrast, METH_O), METHODB(parse_color, METH_O | METH_CLASS), {NULL} /* Sentinel */
};
...
repr(Color *self) {
    if (self->color.alpha)
        return PyUnicode_FromFormat("Color(red=%u, green=%u, blue=%u, alpha=%u)", self->color.r, self->color.g, self->color.b, self->color.a);
...
PyTypeObject Color_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "kitty.fast_data_types.Color",
...
int
init_ColorProfile(PyObject *module) {
