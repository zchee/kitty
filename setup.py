...
def compile_c_extension(
    ...
    unsafe = {'-pthread', '-Werror', '-pedantic-errors', '-O3', '-funroll-loops', '-ffast-math', '-fforce-addr', '-flto=thin'}
...
def build_launcher(args: Options, launcher_dir: str = '.', bundle_type: str = 'source') -> str:
    ...
    if not is_arm:
        cflags += ['-mavx', '-mavx2', '-mavx512f', '-mavx512cd', '-mavx512dq', '-mavx512bw', '-mavx512vl', '-mavx512vnni']
    ldflags.append('-O3')
    ldflags.append('-funroll-loops')
    ldflags.append('-ffast-math')
    ldflags.append('-fforce-addr')
    ldflags.append('-flto')
