...
static bool
xxh64_reset(void *s) {
    return XXH3_64bits_reset(s) == XXH_OK;
}
static void *
xxh64_create(void) {
    void *ans = XXH3_createState();
    if (ans != NULL) xxh64_reset(ans);
    return ans;
}
static bool
xxh64_update(void *s, const void *input, size_t length) {
    return XXH3_64bits_update(s, input, length) == XXH_OK;
}
static uint64_t
xxh64_digest64(const void *s) {
    return XXH3_64bits_digest(s);
}
static uint64_t
xxh64_oneshot64(const void *s, size_t len) {
    return XXH3_64bits(s, len);
}
static void
xxh64_digest(const void *s, void *output) {
...
xxh64_hasher(void) {
    hasher_t ans = {
        ...
        .update = xxh64_update,
        .digest = xxh64_digest,
        .digest64 = xxh64_digest64,
        .oneshot64 = xxh64_oneshot64};
    return ans;
}

static bool
xxh128_reset(void *s) {
    return XXH3_128bits_reset(s) == XXH_OK;
}
static void *
xxh128_create(void) {
    void *ans = XXH3_createState();
    if (ans != NULL) xxh128_reset(ans);
    return ans;
}
static bool
xxh128_update(void *s, const void *input, size_t length) {
    return XXH3_128bits_update(s, input, length) == XXH_OK;
}
static void
xxh128_digest(const void *s, void *output) {
...
xxh128_hasher(void) {
    hasher_t ans = {
        .hash_size = sizeof(XXH128_hash_t),
        .block_size = 64,
        .new = xxh128_create,
        .delete = xxh64_delete,
        .reset = xxh128_reset,
        .update = xxh128_update,
        .digest = xxh128_digest,
    };
    return ans;
}
...
free_rsync(Rsync *r) {
    if (r->hasher.state) {
        r->hasher.delete(r->hasher.state);
        r->hasher.state = NULL;
    }
    if (r->checksummer.state) {
        r->checksummer.delete(r->checksummer.state);
        r->checksummer.state = NULL;
    }
}
...
init_rsync(Rsync *ans, size_t block_size, int strong_hash_type, int checksum_type) {
    ...
    if (ans->hasher_constructor == NULL) {
        free_rsync(ans);
        return "Unknown strong hash type";
    }
    if (ans->checksummer_constructor == NULL) {
        free_rsync(ans);
    ...
    if (ans->hasher.state == NULL) {
        free_rsync(ans);
        return "Out of memory";
    }
    ans->checksummer.state = ans->checksummer.new();
    if (ans->checksummer.state == NULL) {
        free_rsync(ans);
        return "Out of memory";
    }
    return NULL;
...
// Patcher {{{
typedef struct {
    PyObject_HEAD rolling_checksum rc;
...
Patcher_init(PyObject *s, PyObject *args, PyObject *kwds) {
    ...
    if (expected_input_size > 0) { self->rsync.block_size = (size_t)round(sqrt((double)expected_input_size)); }
...
signature_header(Patcher *self, PyObject *a2) {
    ...
    if (dest.len < header_size) { PyErr_SetString(RsyncError, "Output buffer is too small"); }
    uint8_t *o = dest.buf;
    le16enc(o, 0);                          // version
...
sign_block(Patcher *self, PyObject *args) {
    ...
    if (PyObject_GetBuffer(a2, &dest, PyBUF_WRITEABLE) == -1) return NULL;
    if (dest.len < (ssize_t)signature_block_size) { PyErr_SetString(RsyncError, "Output buffer is too small"); }
    self->rsync.hasher.reset(self->rsync.hasher.state);
    if (!self->rsync.hasher.update(self->rsync.hasher.state, src.buf, src.len)) {
        PyErr_SetString(PyExc_ValueError, "String hashing failed");
        return NULL;
    }
...
write_block(Patcher *self, uint64_t block_index, PyObject *read, PyObject *write) {
    ...
    if (!PyLong_Check(ret)) {
        PyErr_SetString(PyExc_TypeError, "read callback function did not return an integer");
        return false;
    }
...
apply_op(Patcher *self, Operation op, PyObject *read, PyObject *write) {
    switch (op.type) {
        case OpBlock: return write_block(self, op.block_index, read, write);
        case OpBlockRange:
        ...
        case OpHash: {
            uint8_t actual[64];
            if (op.data.len != self->rsync.checksummer.hash_size) {
                PyErr_SetString(RsyncError, "checksum digest not the correct size");
                return false;
            }
            ...
            if (memcmp(actual, op.data.buf, self->rsync.checksummer.hash_size) != 0) {
                ...
                PyErr_Format(
                    RsyncError,
                    "Failed to verify overall file checksum actual: %S != expected: %S, this usually happens because one of the involved files was altered "
...
finish_delta_data(Patcher *self, PyObject *args UNUSED) {
    if (self->buf.len > 0) {
        PyErr_Format(RsyncError, "%zu bytes of unused delta data", self->buf.len);
        return NULL;
    }
    if (!self->checksum_done) {
        PyErr_SetString(RsyncError, "The checksum was not received at the end of the delta data");
        return NULL;
    }
    Py_RETURN_NONE;
}
...
static PyObject *
Patcher_block_size(Patcher *self, void *closure UNUSED) {
    return PyLong_FromSize_t(self->rsync.block_size);
}
static PyObject *
Patcher_total_data_in_delta(Patcher *self, void *closure UNUSED) {
    return PyLong_FromSize_t(self->total_data_in_delta);
}

PyGetSetDef Patcher_getsets[] = {
    {"block_size", (getter)Patcher_block_size, NULL, NULL, NULL}, {"total_data_in_delta", (getter)Patcher_total_data_in_delta, NULL, NULL, NULL}, {NULL}};
...
PyTypeObject Patcher_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "rsync.Patcher",
...
static void
free_signature_val(SignatureVal x) {
    free(x.weak_hash_collisions);
}
...
parse_signature_header(Differ *self) {
    ...
    uint32_t x;
    if ((x = le16dec(p)) != 0) {
        PyErr_Format(RsyncError, "Invalid version in signature header: %u", x);
        return;
    }
    p += 2;
    if ((x = le16dec(p)) != 0) {
        PyErr_Format(RsyncError, "Invalid checksum type in signature header: %u", x);
        return;
    }
    p += 2;
    if ((x = le16dec(p)) != 0) {
        PyErr_Format(RsyncError, "Invalid strong hash type in signature header: %u", x);
        return;
    }
    p += 2;
    if ((x = le16dec(p)) != 0) {
        PyErr_Format(RsyncError, "Invalid weak hash type in signature header: %u", x);
        return;
    }
    p += 2;
...
finish_signature_data(Differ *self, PyObject *args UNUSED) {
    if (self->buf.len > 0) {
        PyErr_Format(RsyncError, "%zu bytes of unused signature data", self->buf.len);
        return NULL;
    }
    self->buf.len = 0;
...
ensure_idx_valid(Differ *self, size_t idx) {
    ...
    if (!ret) return false;
    if (!PyLong_Check(ret)) {
        PyErr_SetString(PyExc_TypeError, "read callback did not return an integer");
        return false;
    }
    size_t n = PyLong_AsSize_t(ret);
...
find_strong_hash(const SignatureVal *sm, uint64_t q, uint64_t *block_index) {
    if (sm->sig.strong_hash == q) {
        *block_index = sm->sig.index;
        return true;
    }
    for (size_t i = 0; i < sm->len; i++) {
        if (sm->weak_hash_collisions[i].strong_hash == q) {
            *block_index = sm->weak_hash_collisions[i].index;
            return true;
        }
    }
    return false;
...
enqueue(Differ *self, Operation op) {
    switch (op.type) {
        ...
        case OpHash:
            ...
            return send_op(self, &op);
        case OpBlockRange:
        case OpData: PyErr_SetString(RsyncError, "enqueue() must never be called with anything other than OpHash and OpBlock"); return false;
    }
    return false;
}
...
finish_up(Differ *self) {
    ...
    op.data.buf = self->checksum;
    op.data.len = self->rsync.checksummer.hash_size;
...
next_op(Differ *self, PyObject *args) {
    ...
    if (self->finished && !PyErr_Occurred()) { send_pending(self); }
    self->read = NULL;
    self->write = NULL;
    if (PyErr_Occurred()) return NULL;
...
static PyMethodDef Differ_methods[] = {
    METHODB(add_signature_data, METH_VARARGS), METHODB(finish_signature_data, METH_NOARGS), METHODB(next_op, METH_VARARGS), {NULL} /* Sentinel */
};


PyTypeObject Differ_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "rsync.Differ",
...
Hasher_init(PyObject *s, PyObject *args, PyObject *kwds) {
    ...
    if (self->h.state == NULL) {
        PyErr_NoMemory();
        return -1;
    }
    if (data.buf && data.len > 0) { self->h.update(self->h.state, data.buf, data.len); }
    return 0;
}
...
digest64(Hasher *self, PyObject *args UNUSED) {
    if (self->h.digest64 == NULL) {
        PyErr_SetString(PyExc_TypeError, "Does not support 64-bit digests");
        return NULL;
    }
...
static PyObject *
Hasher_digest_size(Hasher *self, void *closure UNUSED) {
    return PyLong_FromSize_t(self->h.hash_size);
}
static PyObject *
Hasher_block_size(Hasher *self, void *closure UNUSED) {
    return PyLong_FromSize_t(self->h.block_size);
}
static PyObject *
Hasher_name(Hasher *self, void *closure UNUSED) {
...
PyTypeObject Hasher_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "rsync.Hasher",
...
call_ftc_callback(PyObject *callback, char *src, Py_ssize_t key_start, Py_ssize_t key_length, Py_ssize_t val_start, Py_ssize_t val_length) {
    while (src[key_start] == ';' && key_length > 0) {
        key_start++;
        key_length--;
    }
